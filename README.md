...
Some further notes below for installing future systems/features on RHEL

GOAL:
  Connect a user to a full toolchain
		a. Setup git for them and connect them to their master repo
		b. Create a jenkins instance or connect them to an existing one
		c. On checking in a sample app - build it on the jenkins instance, save the output to maven
		c. Give the user the ability to deploy a new vm and install the app onto it - via a jenkins manual job 
		d. Setup their local ide on their windows desktop
...
3. Set up puppet
	...
	# install jenkins on the client/agent machine
	References: 
		http://jenkins-ci.org/
	...
	# bdd testing / monitoring on server
	# http://nokogiri.org/tutorials/installing_nokogiri.html
	# Resources EPEL http://fedoraproject.org/wiki/EPEL#How_can_I_use_these_extra_packages.3F
